...
class AgnosticBrainDecoder {
    ...
    bandpassFilter(data, lowFreq, highFreq) {
        const f1 = lowFreq / (this.samplingRate / 2);
        const f2 = highFreq / (this.samplingRate / 2);
        const filter = new dsp.IIRFilter(dsp.BANDPASS, f1, f2);
        return data.map(x => filter.singleStep(x));
    }

    extractWeightedBandPower(data) {
        let features = [];
        for (let [bandName, [lowFreq, highFreq]] of Object.entries(this.frequencyBands)) {
            let filteredData = this.bandpassFilter(data, lowFreq, highFreq);
            let bandPower = mathjs.mean(mathjs.dotMultiply(filteredData, filteredData));
